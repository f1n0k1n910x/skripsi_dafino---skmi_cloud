...
function applyTranslation(lang) {
  document.querySelectorAll('[data-lang-key]').forEach(el => {
    ...
    if (translations[key]?.[lang]) {
      if (key === 'helloUserGreeting') {
        const username = document.getElementById('userInfoGreeting')?.textContent.split(' ')[1] || '';
        el.textContent = `${translations[key][lang]} ${username}`;
      } else {
  ...
  document.querySelectorAll('[data-lang-title]').forEach(el => {
    ...
    if (translations[key]?.[lang]) el.title = translations[key][lang];
  });

  // Active button
  document.getElementById('langIdBtn')?.classList.toggle('active-lang', lang === 'id');
...
export { translations, applyTranslation, initTranslation, setLanguage, getCurrentLanguage };
